...
const registrarIntento = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Faltan campos obligatorios o respuestas inválidas." });
  }
  ...
  try {
    ...
    if (asignacion.rowCount === 0) {
      await registrarError(
        "warning",
        "Intento inválido: asignación no encontrada",
        "registrarIntentoCuestionario"
      );
    ...
    const intentosPermitidos =
      cuestionarioInfo.rows[0]?.intentos_permitidos ?? 1;
    const intentosHechos = parseInt(intentosPrevios.rows[0].count);

    if (intentosHechos >= intentosPermitidos) {
      return res
        .status(400)
        .json({ error: "Ya alcanzaste el límite de intentos permitidos." });
    }
    ...
    for (const pregunta of preguntas.rows) {
      ...
      if (pregunta.tipo === "unica") {
        ...
        const aciertos = respuestaUsuario.respuestas.filter((r) =>
          correctas.includes(r)
        );
    ...
    // 4. Registrar intento
    const duracion = duracion_segundos; // en segundos, puedes calcularlo si lo pasás desde el frontend
    ...
    // Determinar estado del cuestionario después del intento actual
//const aprobado = nota >= nota_minima;

let estado;
if (aprobado) {
  estado = "aprobado";
} else if (intentosHechos + 1 >= intentosPermitidos) {
  estado = "reprobado";
} else {
  estado = "en_progreso";
}

// Actualizar resultado si es mejor
...
if (resultadoPrevio.rowCount === 0 || nota > resultadoPrevio.rows[0].nota) {
  ...
  // NUEVO → Actualizar usuarios_capacitaciones con el nuevo estado correcto
  await client.query(
    `UPDATE usuarios_capacitaciones
     SET nota_obtenida = $1, aprobado = $2, fecha_completado = CURRENT_TIMESTAMP,
         estado = $3
     WHERE id_asignacion = $4 AND id_empresa = $5`,
    [nota, aprobado, estado, id_asignacion, id_empresa]
  );
} else {
  // Si la nota NO es mejor, igual actualizar el estado para reflejar correctamente el progreso
  await client.query(
    `UPDATE usuarios_capacitaciones
     SET estado = $1
     WHERE id_asignacion = $2 AND id_empresa = $3`,
    [estado, id_asignacion, id_empresa]
  );
}
